...
export function MP3toWAVConverter() {
  ...
  // 修改handleDrop函数，添加更明显的错误日�?  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
    setIsDragging(false);
  ...
  const handleClientSideConversion = async () => {
    ...
    try {
      ...
      // 设置原始文件�?- 添加这一行以修复下载文件名问�?      setOriginalName(file.name);
  ...
  const handleDownload = () => {
    ...
    // 跳转到下载页�?    window.location.href = `/download/${downloadId}`;
  };
  
  // 生成唯一ID的辅助函�?  const generateUniqueId = () => {
            ...
            {!file ? (
              <div
                className={cn(
                  ...
                  errorMessage ? "border-red-500" : "" // 添加错误状态样�?                )}
                onDragOver={(e) => {
                  e.preventDefault()
                  setIsDragging(true)
                }}
